...
def space_to_depth(input, block_size):
    ...
    c1 = (block_size ** 2) * c
    input = input.view(-1, h1, block_size, w1, block_size, c)
    out = input.permute(0, 1, 3, 2, 4, 5).contiguous()
    return out.view(-1, h1, w1, c1)
...
def depth_to_space(input, block_size):
    ...
    c = c1 // (block_size ** 2)
    input = input.view(-1, h1, w1, block_size, block_size, c)
    out = input.permute(0, 1, 3, 2, 4, 5).contiguous()
...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            attn = attn.view(-1, mask.size(1), self.num_heads,
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x = self.attn(x.view(*msa_shape), mask)
        x = depth_to_space(x.view(*wmsa_shape), self.window_size)
        if self.shift_size > 0 and mask is not None:
            x = x.roll((self.shift_size,) * 2, dims=(1, 2))
